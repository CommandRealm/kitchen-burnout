...
execute if score $display_mode settings matches 0 run setblock 5 66 44 oak_wall_sign[facing=north]{front_text:{messages:['{"text":"","color":"blue","bold":true,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 1.. run function options:error"}}', '[" ",{"translate":"Mode: ","color":"gray","bold":false}]', '{"translate":"Classic","color":"gold"}', '{"text":"<<<         >>>","color":"#d4d4d4","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score $game state matches 0 run function options:mode/click_mode"}}'], has_glowing_text: false, color: "black"}}
