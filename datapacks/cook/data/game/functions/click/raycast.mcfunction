...
execute if entity @e[type=marker,tag=prep_display,limit=1,distance=..0.35] unless entity @e[type=armor_stand,tag=prep_slot,distance=..10] unless entity @e[type=villager,tag=customer,tag=update_line_pos,distance=..100] run function game:click/prep_slot_hit

execute if score @s[nbt={SelectedItemSlot:1}] map matches 11..15 if block ~ ~ ~ #game:block_fish run scoreboard players set $calculate calculate 0
