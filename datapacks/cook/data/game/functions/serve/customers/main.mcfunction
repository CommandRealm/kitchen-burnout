...
execute if score @s[tag=!tutorial] customers matches 0 if score $mode settings matches 2 unless entity @e[type=villager,tag=customer,tag=update_line_pos,distance=..100] unless score $winner shuffle matches 1.. run function game:serve/customers/summon
# Move customers
