...
execute store result score $players end if entity @a[gamemode=adventure,tag=playing,tag=!tutorial,scores={end=1..}]

# Let other players know someone is trying to end the game
execute if score $players end > $old_players end unless score $players end >= $required end run tellraw @a[gamemode=adventure,tag=playing,tag=!tutorial] ["",{"text":"\n- ","color":"gray"},{"score":{"name":"$players","objective":"end"},"color":"dark_aqua"},{"translate":" out of ","color":"green"},{"score":{"name":"$required","objective":"end"},"color":"dark_aqua"},{"translate":" required players would like to end the game!","color":"green"}]
execute if score $players end > $old_players end unless score $players end >= $required end as @a[gamemode=adventure,tag=playing,tag=!tutorial] unless score @s end matches 1.. run tellraw @s ["",{"text":"- ","color":"gray"},{"translate":"If you'd like to end the game early, click this message or run","color":"gray","clickEvent":{"action":"run_command","value":"/trigger end"}},{"text":"\n- ","color":"gray"},{"text":"/trigger end","clickEvent":{"action":"run_command","value":"/trigger end"},"color":"dark_gray","underlined":true},{"text":"\n"}]
execute store result score $old_players end if entity @a[gamemode=adventure,tag=playing,tag=!tutorial,scores={end=1..}]
