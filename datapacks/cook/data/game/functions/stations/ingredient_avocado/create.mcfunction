...
setblock ~ ~ ~ glass
summon marker ~ ~ ~ {Tags:["ingredient_box","reset_boxes","die_between_games","raycast_target","18"]}
scoreboard players set @e[type=marker,sort=nearest,limit=1,tag=ingredient_box,distance=..1] ingredient 18
# Glowing outline
execute if score $glowing settings matches 1 run summon magma_cube ~ ~ ~ {Silent:1b,Invulnerable:1b,Glowing:1b,NoAI:1b,Size:1,PersistenceRequired:1b,Tags:["die_between_games","ingredient_box_outline","invisible","avocado_slice"],ActiveEffects:[{Id:14,Amplifier:1b,Duration:1000000,ShowParticles:0b}]}
