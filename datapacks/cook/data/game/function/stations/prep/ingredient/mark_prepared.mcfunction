...
scoreboard players remove $required serving 1
execute store result storage game:stations/prep prepared_ingredient.required int 1 run scoreboard players get $required serving
...
$execute if score $required serving matches 2.. run \
    data modify entity @s data.recipe_icons[$(index)].extra[1].text set string \
        storage game:stations/prep prepared_ingredient.required
