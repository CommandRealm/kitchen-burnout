# Called by a cutting board that has been clicked on by a player
...
data modify storage game:cutting_board/take id set from entity @n[type=item_display,tag=cutting_board,distance=..0.5] item.components."minecraft:custom_data".ingredient
...
execute on target run function game:inventory/set_player_ingredient with storage game:cutting_board/take
