# Called by a stovetop that has been right clicked on by a player to take, place, or swap an ingredient
...
execute unless data entity @n[type=item_display,tag=stovetop,distance=..0.5] item run return run function game:stations/stovetop/place_ingredient

## BELOW HERE MEANS THAT WE HAD AN INGREDIENT TO BEGIN WITH
# Cache our ingredient
data modify storage game:stations/take id set from entity @n[type=item_display,tag=stovetop,distance=..0.5] item.components."minecraft:custom_data".ingredient
...
execute if score $holding_ingredient station matches 1 run function game:stations/stovetop/place_ingredient
...
# Clear the ingredient from the stovetop if the player didn't give us one
