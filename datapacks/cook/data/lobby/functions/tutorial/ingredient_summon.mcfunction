...
execute if data storage tutorial Recipe{Ingredients:["Hamburger Bun"]} run summon marker ~ 0 ~ {Tags:["recipe_ingredient","hamburger_bun","tutorial"],CustomName:'["",{"translate":"Hamburger Bun"}]'}
...
execute if data storage tutorial Recipe{Ingredients:["Hamburger Bun"]} run tag @e[type=#game:ingredient_box_outline,tag=ingredient_box_outline,tag=hamburger_bun,tag=tutorial] add recipe_ingredient
